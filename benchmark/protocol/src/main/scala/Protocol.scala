...
object protocol {
  ...
  implicit lazy val smallWCodec: Codec[SmallW] =
    (map(utf8,utf8) ~~ list(utf8)).widenAs[SmallW](SmallW.apply, SmallW.unapply)
  
  implicit lazy val mediumWCodec: Codec[MediumW] =
    (int32 ~~ utf8 ~~ list[SmallW](smallWCodec) ~~ optional(int32)).widenAs(MediumW.apply, MediumW.unapply)
  
  implicit lazy val largeWCodec: Codec[LargeW] =
    (int32 ~~ list(utf8) ~~ utf8 ~~ map(utf8,utf8) ~~ list(mediumWCodec) ~~ indexedSeq[SmallW](smallWCodec)).widenAs(LargeW.apply, LargeW.unapply)
  
  implicit lazy val bigWCodec: Codec[BigW] =
